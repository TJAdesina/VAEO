...
def _handle_input(user_text: str, rerun_after: bool = False) -> None:
    """
    ...
    with st.chat_message("assistant"):
        ...
        if wav_bytes:
            if rerun_after:
                # For voice input we call st.rerun() next; queue audio for then
                st.session_state.tts_queue = wav_bytes
            else:
                # For text input there is no rerun; inject autoplay immediately
...
text_input = st.chat_input("Type your farming question…")
if text_input:
    _handle_input(text_input, rerun_after=False)
...
st.caption(
    "Click **Start Recording**, speak your question, then click **Stop Recording**."
)
...
if audio_result:
    ...
    if not user_text:
        ...
        _handle_input(user_text, rerun_after=True)
