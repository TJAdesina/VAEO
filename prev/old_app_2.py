...
# ── Input bar: mic beside the chat input ─────────────────────────────────────
# mic_recorder is rendered here in the normal page flow so Streamlit can
# communicate with it, but CSS lifts it into the fixed footer next to
# st.chat_input. Because it never moves relative to the page DOM — it is
# always the first component after the conversation — it never gets displaced
# by growing chat history, eliminating the white-rectangle corruption bug.

st.markdown(
    """
    <style>
    /* Shrink the chat-input bar's right padding to make room for the mic */
    .stChatInputContainer {
        padding-right: 3.5rem !important;
    }
    /* Grab the mic iframe and pin it into the footer, right of the text field */
