...
# ── Render mic FIRST (stable DOM position) ───────────────────────────────────
# mic_recorder must be rendered before any dynamic content (_render_history,
# _handle_input) so its iframe always occupies the same position in the DOM on
# every rerun. If it were placed after the history it would shift downward as
# new messages are added, causing Streamlit's reconciler to lose the iframe
# reference and replace it with a blank white rectangle.
# CSS then lifts it visually to sit between the chat history and the footer.

st.markdown(
    """
    <style>
    /* Extra bottom padding so history doesn't hide behind the two-row footer */
    .main .block-container {
        padding-bottom: 7rem !important;
    }
    /* Pin the mic iframe just above the chat-input footer */
    iframe[title="streamlit_mic_recorder.mic_recorder"] {
        position  : fixed        !important;
        bottom    : 3.6rem       !important;   /* sits above the chat-input bar  */
        left      : 50%          !important;
...
# ── Text input (renders in fixed footer) ─────────────────────────────────────

text_input = st.chat_input("Type or speak your farming question…")
if text_input:
    _handle_input(text_input)

# ── Handle voice input (audio_result captured above, processed after history) ─
