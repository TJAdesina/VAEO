...
and audio players can consume it directly.
"""
...
def synthesize_speech(text: str, language_name: str) -> bytes:
    ...
    Internal use only — callers should use synthesize_speech_parallel()
    which handles WAV wrapping.
    """
    ...
    try:
        return _client.speech.generate(
            text=text,
            language=cfg.get("spitch_code", cfg["code"]),
...
def synthesize_speech_parallel(text: str, language_name: str) -> bytes:
    ...
    # Concatenate in sentence order, wrap once in a WAV header
