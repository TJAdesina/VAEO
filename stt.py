...
Reliability strategy:
  ...
  3. On first attempt use GEMINI_MODEL_PRIMARY (lighter, higher availability).
  4. If Gemini returns a 503 / "high demand" error, automatically retry once
     with GEMINI_MODEL_FALLBACK (the heavier fallback model).
...
language_name accepted values:
  Any value from LANGUAGES[x]["gemini_lang"] in config.py, e.g.:
  "English", "Nigerian Pidgin", "Yoruba", "Igbo", "Hausa"
  The prompt instructs Gemini to transcribe in that language without translation.
"""
...
def _upload_audio(audio_bytes: bytes, mime_type: str) -> tuple[str, str]:
    """
    Upload audio bytes to the Gemini Files API.

    Returns (file_uri, file_name).  The file_name is used for cleanup.
    """
...
def transcribe_audio(
    ...
    Parameters
    ----------
    audio_bytes   : raw audio data (WAV / WebM / MP3 / OGG etc.)
    mime_type     : MIME type string, e.g. "audio/wav", "audio/webm"
    language_name : gemini_lang value from config.LANGUAGES, e.g. "Nigerian Pidgin"
...
"""
    ...
    try:
        for model in models_to_try:
            for attempt in range(1, GEMINI_MAX_RETRIES + 1):
                try:
                    ...
                    logger.info("STT success (model=%s, attempt=%d)", model, attempt)
                    return text
        ...
        raise RuntimeError(
            f"STT failed after all retries and fallback models. Last error: {last_exc}"
        )
